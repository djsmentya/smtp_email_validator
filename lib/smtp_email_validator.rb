...
module SmtpEmail
    module Validator
        class SmtpEmailValidator < ActiveModel::EachValidator
            def validate_each(record, attribute, value)
                unless Email::Check.run(value,nil, Mail::Address.new(value).domain).valid?
                    record.errors[attribute] << (options[:message] || "is not exists. Double check your email!")
                end
            end
        end
    end
